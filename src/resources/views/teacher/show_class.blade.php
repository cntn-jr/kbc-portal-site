...
@section('class_content')
    ...
    <curriculum-component :csrf="{{json_encode(csrf_token())}}" :curriculum="{{json_encode($curriculum)}}" curriculum_store_url="{{route('curriculum.store', $class->id)}}" :class_lessons="{{json_encode($class_lessons)}}" :class_id="{{$class->id}}" :is_class_teacher="1" />
@endsection