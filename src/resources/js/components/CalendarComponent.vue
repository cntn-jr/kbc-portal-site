...
export default {
    components: { AddSchedule, ScheduleComponent },
    data(){
        return{
            current: moment(),
            is_add_modal: false,
    ...
    methods:{
        ...
        getThisMonth(){
            ...
            for(let day = 1; day <= endDate; day++){
                if(current.year() == moment().year() && current.month() == moment().month() && day === moment().date()){
    ...
    props: ['is_teacher', 'csrf', 'redirect_pass', 'schedules', 'class_id'],
}
</script>
